    ...
    if primeiro_free == True:
        if n_mulligan == 0:
            ...
            for i in range (2,n_mulligan+1):
                res_prod = 1
                for j in range (1,i):
                    res_prod=res_prod*(1-prob_range_land(lands_min,lands_max,cards_mao-i+j+1, lands, cards_tot))
                res = res+(1-prob_range_land(lands_min,lands_max,cards_mao, lands, cards_tot))*res_prod*prob_range_land(lands_min,lands_max,cards_mao-(i-1), lands, cards_tot)

    else :
        for i in range (1,n_mulligan+1):
            res_prod = 1
            for j in range (0,i):
                res_prod=res_prod*(1-prob_range_land(lands_min,lands_max,cards_mao-i+j+1, lands, cards_tot))
            res=res+res_prod*prob_range_land(lands_min,lands_max,cards_mao-i, lands, cards_tot)
...
def mulligan_fixo(lands_min,lands_max,n_mulligan, cards_tot, primeiro_free):
    ...
    top_jumps_val=sorted(delta)[-20:] #valores dos 20 maiores saltos
    for j in range(0,20):
        top_jumps_pos.append(delta.index(top_jumps_val[19-j])+1) #posição dos 3 maiores saltos
    return x,y, top_jumps_pos


#plt.show(), print(f'Biggest jumps in probability happen at these thresholds, in order: {top_jumps_pos}')
...
if radio_box == "Info":
    ...
    st.markdown("You can check the source code of the application at https://github.com/Cromato/ManaBase.")
    ...
    st.latex("P(range|n\_mulligan)=P(range)+(1-P(range))P(range)")
    st.latex("+\displaystyle\sum_{i=2}^{n\_mulligan}\prod_{j=1}^{i-1}(1-P(range))(1-P(range|cards\_mao-i+j+1))\cdot P(range|cards\_mao-i+1)")
    st.markdown("The code used for this function is:")
    st.code('''def prob_fin(lands_min,lands_max,n_mulligan,lands, cards_tot, primeiro_free):
    res=prob_range_land(lands_min,lands_max,cards_mao, lands, cards_tot)
